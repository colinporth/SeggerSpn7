...
// Dual motor Frequency comparison definition */
#define SAME_FREQ   0u
...
// Two components stator current type definition
typedef struct {
	int16_t qI_Component1;
	int16_t qI_Component2;
	} Curr_Components;

// Two components stator voltage type definition
typedef struct {
	int16_t qV_Component1;
	int16_t qV_Component2;
	} Volt_Components;

// ADConv_t type definition, it is used by PWMC_ADC_SetSamplingTime method of PWMnCurrFdbk class for user defined A/D regular conversions
typedef struct {
...
// SensorType_t type definition, it's used in BusVoltageSensor and TemperatureSensor component parameters structures
// to specify whether the sensor is real or emulated by SW
typedef enum {
	REAL_SENSOR, VIRTUAL_SENSOR
	} SensorType_t;

// DOutputState_t type definition, it's used by DOUT_SetOutputState method of DigitalOutput class to specify the
//     required output state
typedef enum {
	INACTIVE, ACTIVE
	} DOutputState_t;

// STC_Modality_t type definition, it's used by STC_SetControlMode and STC_GetControlMode methods in
...
// IMFF_PMSM class, structure type definition for feed-forward constants tuning
typedef struct {
	int32_t wConst_1D;
	int32_t wConst_1Q;
	int32_t wConst_2;
...
// Current references source type, internal or external to FOCDriveClass
typedef enum {
	INTERNAL, EXTERNAL
	} CurrRefSource_t ;

// FOC variables structure
typedef struct {
...
// MPInfo structure (used only for serial communication)
typedef struct {
	uint8_t* data;
	uint8_t len;
	} MPInfo_t, *pMPInfo_t;

// UserInterface related exported definitions */
...
// PFC related exported definitions */
...
// Definitions exported for the DAC channel used as reference for protection */
